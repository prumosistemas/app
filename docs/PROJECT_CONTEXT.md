...
- API alvo atual: 1.0.92, preservando autenticação direta no ThinkPad e seleção adaptativa entre as rotas de resolução antes do fallback residencial.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.92`; a imagem em produção imediatamente anterior deve ser mantida como rollback local até a validação do deploy.
