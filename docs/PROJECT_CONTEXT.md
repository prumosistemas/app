...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.71`; manter a 1.0.70 como rollback local.
...
- Testes locais: 147 aprovados na versão 1.0.71; incluem janela vazia legítima, backoff persistente, preferência adaptativa de Modal, retomada sem reindexar, checkpoint parcial, temporal multi-etapas, failover e isolamento por colaborador.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.71 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.70 como rollback local.
