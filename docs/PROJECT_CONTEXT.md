...
- API alvo: 1.0.61, com autenticação mTLS direta no ThinkPad, Modal principal, segunda conta Modal e fallback residencial do solver.
...
- Solver Portal: Google Modo IA v22 unificado. Desafios são classificados como estáticos, temporais completos, trajetória ou desconhecidos. Perguntas que dependem do ciclo inteiro usam 16 quadros em montagem cronológica; uma cena complexa repetida além do limite é trocada pelo refresh do hCaptcha. A conta `ryangurgell20` mantém um container e um buffer; `fabriciofarofa5` escala a zero e e usada em quota/indisponibilidade. Falha visual especifica segue ao ThinkPad sem colocar as outras notas em cooldown. Modal mantém 90 s por solve e o fallback residencial usa até 240 s. Não há Florence, Cohere nem resolvedor separado para grade de nove imagens.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.61`; manter a 1.0.60 como rollback local.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.61 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.60 como rollback local.
