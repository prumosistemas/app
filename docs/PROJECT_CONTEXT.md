...
- Solver Portal: Google Modo IA v27 unificado. Desafios são classificados como estáticos, temporais completos, trajetória ou desconhecidos. Perguntas que dependem do ciclo inteiro observam até 80 quadros/~17,4 s e só encerram cedo depois de movimento comprovado, ao menos dez segundos e retorno relativo ao estado inicial; isso evita confundir a pequena área da abelha com um ciclo completo. O Modo IA recebe uma única sobreposição na geometria original com fundo temporal e heatmap de ocupação: passagem rápida fica fraca e permanência/pouso fica forte; os quadros extras são processados em streaming no container e a montagem cronológica permanece nos artefatos de depuração. Uma cena complexa repetida além do limite é trocada pelo refresh do hCaptcha. O congelamento usado durante a análise é removido antes do clique, preservando o processamento de interação do widget. A conta `ryangurgell20` mantém um container e um buffer; `fabriciofarofa5` escala a zero e e usada em quota/indisponibilidade. Falha visual especifica segue ao ThinkPad somente como último fallback, sem colocar as outras notas em cooldown. Modal mantém 90 s por solve e o fallback residencial usa até 240 s. Não há Florence, Cohere nem resolvedor separado para grade de nove imagens.
