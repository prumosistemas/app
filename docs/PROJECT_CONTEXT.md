...
- API alvo: 1.0.69, com autenticação mTLS direta no ThinkPad, Modal principal, segunda conta Modal antes do fallback residencial do solver.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.69`; manter a 1.0.68 como rollback local.
...
- Testes locais: 138 aprovados na versão 1.0.69; incluem período em janelas, concorrência automática, checkpoint parcial, health frio, cooldown por causa e rota, renovação antecipada de widget vazio, temporal multi-etapas, preservação de timeout, auto-rearme do circuito, failover e isolamento Alan/Gabriel.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.69 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.68 como rollback local.
