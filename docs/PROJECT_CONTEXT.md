...
- API alvo atual: 1.0.105, preservando autenticação direta no ThinkPad e seleção adaptativa entre HF, as contas Modal e o fallback residencial.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.105`; o deploy mantém automaticamente a atual e as duas anteriores como rollback local.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.105 é construída diretamente no ThinkPad após `git pull`. O deploy mantém somente a atual e duas anteriores como rollback local.
