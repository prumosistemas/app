...
- API alvo: 1.0.67, com autenticação mTLS direta no ThinkPad, Modal principal, segunda conta Modal antes do fallback residencial do solver.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.67`; manter a 1.0.66 como rollback local.
...
- Testes locais: 137 aprovados na versão 1.0.67; incluem período em janelas, concorrência automática, checkpoint parcial, health frio, cooldown por causa e rota, renovação antecipada de widget vazio, preservação de timeout, auto-rearme do circuito, failover e isolamento Alan/Gabriel.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.67 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.66 como rollback local.
