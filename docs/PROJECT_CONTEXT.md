...
- A aba `Notas automático` também lista cada captura preservada: certificado, período, status, quantidade pronta/total, erros, pendências, competências e horários de criação/finalização. O botão `Baixar ZIP` usa os mesmos artefatos persistidos da run e mantém a separação entre recebidas, emitidas, competências, XML e PDF. `Capturar agora` permanece nessa aba e mostra o andamento no histórico automático.
