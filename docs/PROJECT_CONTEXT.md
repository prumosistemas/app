...
- API alvo: 1.0.54, com autenticação mTLS direta no ThinkPad, Modal principal, segunda conta Modal e fallback residencial do solver.
...
- Portal Alan/SIM7: a prova completa de 01/06 a 17/07 finalizou 374/374, com janelas 169/169 e 205/205, zero duplicata e zero erro final. A execução anterior levou 8h26 e iniciou itens 546 vezes; a 1.0.54 preserva XML concluído quando o PDF falha, persiste a sessão Modo IA recuperada, não derruba o pool por captura transitória e tenta a conta reserva quando a causa real é a sessão do provedor.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.54`; manter a 1.0.53 como rollback local até concluir a prova real.
...
- Testes locais: 87 aprovados na versão 1.0.54; incluem período em janelas, concorrência automática, checkpoint parcial, failover e isolamento Alan/Gabriel.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.54 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.53 como rollback local.
