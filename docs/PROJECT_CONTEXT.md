...
## Estado validado em 2026-09-08

### Operação 1.0.113 em 2026-09-08

- Qwen3-VL 235B via HF Inference Providers passou em 4/4 evidências temporais
  reais com token já confirmado. Três provas do adaptador final levaram entre
  3,34 s e 9,89 s; a rota ficou restrita a temporais completos porque falhou
  em dois desafios estáticos de formatos diferentes.
- Os tokens HF primário e secundário formam um pool independente dos seis
  Spaces. Quota, baixa confiança e falha de transporte retornam à cadeia Google
  existente; 402/403 põe somente a conta afetada em cooldown.
- Durante o foco de agosto, o agendador retoma a run deferida do certificado
  antes de criar uma duplicata diária. Todos os arquivos e índices válidos do
  checkpoint são preservados.

### Operação 1.0.107 em 2026-08-25

- A telemetria real mostrou que probes visuais a cada 60 s chegavam antes do
  fim do cooldown de 90--106 s dos Spaces, acumulando 27 falhas e custo sem
  ganho de disponibilidade. Bloqueio Google/circuito/429 agora usa
  30/60/90/120 s; transporte comum continua limitado a 60 s.
- Duas contas Modal estavam em HTTP 404 por indisponibilidade da conta e a
  terceira permaneceu funcional. Checkpoints novos foram indexados e
  preservados, sem erro de nota, enquanto a sonda global recupera a cadeia.

### Operação 1.0.106 em 2026-08-24

- A observacao real do hedge revelou conexoes Modal vivas alem do orçamento:
  o timeout numerico de `requests` era renovado por bytes intermediarios. O
  cliente do solver agora usa deadline total do `aiohttp`, inclusive corpo.
- Uma resposta lenta continua tendo o prazo visual longo, mas nao conserva
  threads depois do teto da cadeia. Teste com servidor que envia um byte a
  cada 10 ms comprova o corte absoluto. Validacao local: 238 testes Pytest.

### Operação 1.0.105 em 2026-08-24

- Uma rota remota lenta nao consome mais quase todo o prazo antes da reserva.
  A segunda conta Modal entra apos 30 s, com limite de duas tentativas para o
  captcha; resposta abaixo da janela nao cria custo adicional.
- O ThinkPad continua fora da disputa e so e consultado depois das rotas
  remotas. O backoff global foi reduzido para 30/45/60 s, ainda sob lease unico.
- Provedor, duracao e uso do hedge ficam no item XML/PDF. Validacao local: 237
  testes Pytest, incluindo vencedor rapido, ausencia de hedge no caminho
  saudavel e teto de um minuto no circuito global.

### Operação 1.0.104 em 2026-08-24

- O circuito de indisponibilidade do Portal passou de local por subprocesso
  para global e persistente. Cooldowns e lease ficam em `_api_data`, de modo
  que deploy, retomada e alternancia de colaborador nao reiniciam uma rajada de
  quatro navegadores contra as mesmas rotas bloqueadas.
- Uma run por vez sonda a cadeia; as demais preservam checkpoint. O primeiro
  token reabre a concorrencia em duas tarefas e o sucesso seguinte restaura
  quatro. Runs manuais aguardam o lease sem furar o circuito.
- O indice registra provedor e duracao por XML/PDF. O Modal informa se a
  analise veio de um Space HF ou do egress direto; o cliente acrescenta a conta
  Modal logica ou `thinkpad`, sempre sem token, URL sensivel ou imagem.
- Validacao local: 234 testes Pytest, incluindo cooldown entre subprocessos,
  lease exclusivo, heartbeat posterior ao outage e rampa 1→2→4.

### Operação 1.0.103 em 2026-08-24

- O terceiro Modal (`prumo-sistema`) esta integrado ao Portal e ao pool ISS,
  com resolvedor e Browserless implantados. O primario sem credito continua em
  quarentena temporaria e volta automaticamente quando responder novamente.
- O desafio visual recente era o mesmo contrato de ponto livre, com a pergunta
  de encontrar o animal diferente. A selecao e o clique estavam corretos, mas o
  relogio sintetico do CDP deixava a transicao posterior incoerente. Ele foi
  removido; a validacao ao vivo permanece e a nova etapa e reconhecida por uma
  assinatura compacta do canvas/DOM, sem reclicar o checkbox.
- O fallback residencial respeita o `retry_after` do circuito local. A auditoria
  resume apenas a chamada HTTP mais recente e exibe somente as tres tentativas
  visuais recentes por nota, sem apagar a retencao de sete dias.
- Requisicoes excedentes deixam de aguardar a trava interna de um Space HF
  ocupado: tentam imediatamente a proxima rota. A conta HF secundaria segue sem
  Spaces porque o provedor recusou a criacao gratuita com HTTP 402.
...
- O Portal mantem quatro downloads em condicao saudavel, mas fecha o pool
  ...
  recuperacao com backoff crescente; sucesso reabre o pool sem consumir as
  tentativas das demais notas.
- Um keepalive da sessao evita expirar cookies durante captchas demorados.
  Cenas estaticas usam um quadro coerente congelado ate o clique, enquanto
  abelha, trajetoria e outros desafios temporais preservam suas sequencias.
...
- API alvo atual: 1.0.113, preservando autenticação direta no ThinkPad e seleção adaptativa entre Qwen/HF, as contas Modal e o fallback residencial.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.113`; o deploy mantém automaticamente a atual e as duas anteriores como rollback local.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.113 é construída diretamente no ThinkPad após `git pull`. O deploy mantém somente a atual e duas anteriores como rollback local.
- O resolvedor anterior foi removido. O caminho geral do hCaptcha continua sendo o Google Modo IA versionado em `solver/google_ai_mode`; Qwen3-VL atende apenas a especialização temporal. A proxy do servidor só poderá ser ativada após autenticação de máquina no Cloudflare Access.
