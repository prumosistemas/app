...
- API alvo: 1.0.73, com autenticação mTLS direta no ThinkPad, seleção adaptativa entre duas contas Modal antes do fallback residencial do solver.
...
- Portal 1.0.77: a captura temporal caiu de uma mediana anterior de aproximadamente 26 s para 11-13 s nas amostras normais, mantendo 30 quadros em 8,7 s. A evidência de ocupação continua síncrona para acertividade; montagem/overlay redundantes deixaram de ser gerados e o MP4 de auditoria usa uma única thread em segundo plano, evitando disputar CPU com a etapa seguinte. Retomadas sem parâmetros preservam XML/PDF da run original, e canvas temporal vazio agora troca de cena/faz failover cedo.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.73`; manter a 1.0.72 como rollback local.
