...
- API alvo: 1.0.63, com autenticação mTLS direta no ThinkPad, Modal principal, segunda conta Modal antes do fallback residencial do solver.
...
- Solver Portal: Google Modo IA v30 unificado. Desafios são classificados como estáticos, temporais completos, trajetória ou desconhecidos. Perguntas que dependem do ciclo inteiro observam até 80 quadros/~17,4 s e só encerram cedo depois de movimento comprovado, ao menos dez segundos e retorno relativo ao estado inicial; isso evita confundir a pequena área da abelha com um ciclo completo. O Modo IA recebe uma única sobreposição na geometria original com fundo temporal e heatmap de ocupação: passagem rápida fica fraca e permanência/pouso fica forte; os quadros extras são processados em streaming no container e a montagem cronológica permanece nos artefatos de depuração. Uma cena complexa repetida além do limite é trocada pelo refresh do hCaptcha. O iframe permanece com `requestAnimationFrame` e temporizadores nativos durante toda a análise; o alvo estático é relocalizado no quadro atual antes do clique. O documento mínimo do widget é injetado por CDP mantendo a origem oficial `https://www.nfse.gov.br`, pois a v28 identificava corretamente os alvos, mas o hCaptcha recusava a resposta emitida na página local. Na v30, o script do hCaptcha é carregado e renderizado explicitamente depois da troca do documento, com CSP liberada apenas na página isolada do solver; resets reinjetam o widget sem perder a origem. A origem recebida é limitada aos hosts oficiais. A conta `ryangurgell20` mantém um container e um buffer; `fabriciofarofa5` escala a zero e e usada em quota/indisponibilidade. Falha visual especifica segue ao ThinkPad somente como último fallback, sem colocar as outras notas em cooldown. Modal mantém 90 s por solve e o fallback residencial usa até 240 s. Não há Florence, Cohere nem resolvedor separado para grade de nove imagens.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.63`; manter a 1.0.62 como rollback local.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.63 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.62 como rollback local.
