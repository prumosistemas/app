...
- Solver Portal: Google Modo IA v42 unificado. Desafios são classificados como estáticos, temporais completos, trajetória ou desconhecidos. Perguntas que dependem do ciclo inteiro observam até 80 quadros/~17,4 s; o Modo IA recebe uma única sobreposição na geometria original com heatmap de ocupação, em que passagem rápida fica fraca e permanência/pouso fica forte. A página oficial da NFS-e é preservada como contexto do widget e a URL aceita é limitada aos hosts oficiais. O hCaptcha visível tenta checkbox, execução oficial e recuperação CDP; se o iframe abrir sem grade, canvas, tarefas ou checkmark em duas leituras, o perfil é renovado cedo. A cena conhecida da abelha pode atravessar até oito etapas com o mesmo fundo, pois cada uma tem trajetória própria; desafios desconhecidos continuam sendo trocados cedo. A conta `ryangurgell20` mantém um container e um buffer; `fabriciofarofa5` escala a zero e é usada em quota/indisponibilidade. Modal e ThinkPad têm teto de 360 s para concluir a sequência antes de repetir em outro endpoint. Timeouts são preservados como `solve_timeout`, sem virar erro genérico de grade. Não há Florence, Cohere nem resolvedor separado para grade de nove imagens.
