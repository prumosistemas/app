...
- API alvo atual: 1.0.96, preservando autenticação direta no ThinkPad e seleção adaptativa entre HF, as duas contas Modal e o fallback residencial.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.96`; o deploy mantém automaticamente a atual e as duas anteriores como rollback local.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.96 é construída diretamente no ThinkPad após `git pull`. O deploy mantém somente a atual e duas anteriores como rollback local.
