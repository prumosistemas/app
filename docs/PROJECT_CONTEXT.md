...
- API alvo: 1.0.70, com autenticação mTLS direta no ThinkPad, duas contas Modal balanceadas antes do fallback residencial do solver.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.70`; manter a 1.0.69 como rollback local.
...
- Testes locais: 141 aprovados na versão 1.0.70; incluem janela vazia legítima, balanceamento Modal, concorrência automática, checkpoint parcial, temporal multi-etapas, failover e isolamento por colaborador.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.70 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.69 como rollback local.
