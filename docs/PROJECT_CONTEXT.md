...
### Operação 1.0.102 em 2026-08-24
...
- O desafio visual recente era o mesmo contrato de ponto livre, com a pergunta
  de encontrar o animal diferente. Ele mudava os animais durante a latencia da
  IA apesar de parecer estatico no primeiro quadro. O solver agora pausa o
  relogio virtual do iframe entre captura e clique, mantendo a validacao ao vivo.
...
- API alvo atual: 1.0.102, preservando autenticação direta no ThinkPad e seleção adaptativa entre HF, as contas Modal e o fallback residencial.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.102`; o deploy mantém automaticamente a atual e as duas anteriores como rollback local.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.102 é construída diretamente no ThinkPad após `git pull`. O deploy mantém somente a atual e duas anteriores como rollback local.
