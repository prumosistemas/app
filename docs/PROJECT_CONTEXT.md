...
- API alvo: 1.0.68, com autenticação mTLS direta no ThinkPad, Modal principal, segunda conta Modal antes do fallback residencial do solver.
...
- Solver Portal: Google Modo IA v43 unificado. Desafios são classificados como estáticos, temporais completos, trajetória ou desconhecidos. Perguntas que dependem do ciclo inteiro observam até 80 quadros/~17,4 s; o Modo IA recebe uma única sobreposição na geometria original com heatmap de ocupação, em que passagem rápida fica fraca e permanência/pouso fica forte. A página oficial da NFS-e é preservada como contexto do widget e a URL aceita é limitada aos hosts oficiais. O hCaptcha visível tenta checkbox, execução oficial e recuperação CDP; se o iframe abrir sem grade, canvas, tarefas ou checkmark em duas leituras, o perfil é renovado cedo. A cena conhecida da abelha pode atravessar até oito etapas com o mesmo fundo, pois cada uma tem trajetória própria; desafios desconhecidos continuam sendo trocados cedo. A conta `ryangurgell20` mantém um container e um buffer; `fabriciofarofa5` escala a zero e é usada em quota/indisponibilidade. Modal e ThinkPad têm teto de 360 s para concluir a sequência antes de repetir em outro endpoint. Uma conta que não forma sessão do Modo IA abre o circuito após três falhas consecutivas e recebe cooldown de cinco minutos no servidor. Timeouts são preservados como `solve_timeout`, sem virar erro genérico de grade. Não há Florence, Cohere nem resolvedor separado para grade de nove imagens.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.68`; manter a 1.0.67 como rollback local.
...
- Testes locais: 138 aprovados na versão 1.0.68; incluem período em janelas, concorrência automática, checkpoint parcial, health frio, cooldown por causa e rota, renovação antecipada de widget vazio, temporal multi-etapas, preservação de timeout, auto-rearme do circuito, failover e isolamento Alan/Gabriel.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.68 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.67 como rollback local.
