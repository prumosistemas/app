...
- API alvo: 1.0.62, com autenticação mTLS direta no ThinkPad, Modal principal, segunda conta Modal e fallback residencial do solver.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.62`; manter a 1.0.61 como rollback local.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.62 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.61 como rollback local.
