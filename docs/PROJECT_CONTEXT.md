...
- API alvo: 1.0.55, com autenticação mTLS direta no ThinkPad, Modal principal, segunda conta Modal e fallback residencial do solver.
...
- Portal Alan/SIM7: a prova completa de 01/06 a 17/07 finalizou 374/374, com janelas 169/169 e 205/205, zero duplicata e zero erro final. A execução anterior levou 8h26 e iniciou itens 546 vezes; a 1.0.55 preserva XML concluído quando o PDF falha, persiste a sessão Modo IA recuperada, não derruba o pool por captura transitória, tenta a conta reserva pela causa real, não desvia a run inteira por health frio e evita recovery repetido de uma sessão Google confirmadamente indisponível.
...
- Imagem alvo do servidor: `ryang20/prumo-api:1.0.55`; manter a 1.0.54 como rollback local até concluir a prova real.
...
- Testes locais: 89 aprovados na versão 1.0.55; incluem período em janelas, concorrência automática, checkpoint parcial, health frio, cooldown por causa, failover e isolamento Alan/Gabriel.
...
- O registro Docker externo não é necessário no caminho normal: a imagem 1.0.55 pode ser construída diretamente no ThinkPad após `git pull`. Manter a 1.0.54 como rollback local.
