...
A conta secundaria nao tinha Spaces. A tentativa de criar
`jorjoinho/navegador-headless-prumo` recebeu HTTP 402: a API informou que a
conta nova deve aguardar 30 dias ou assinar PRO para hospedar ZeroGPU. Nenhum
recurso parcial foi criado e nao houve cobranca.
...
O endpoint da Prumo usa CPU e nao chama o probe decorado com `spaces.GPU`; o
fluxo normal nao consome minutos GPU ZeroGPU. O hardware gratuito pode dormir e
ter cold start. Nao selecionar hardware pago sem autorizacao.
...
Depois de ambos ficarem `RUNNING`, adicione os IDs ao pool e sincronize o Secret
Modal nas duas contas. Ate la, o token secundario permanece somente no cofre e
nao e enviado ao Modal.
