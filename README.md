...
Versão: **1.0.30 - Login leve, homologacao separada e URLs limpas**
...
| `cloudflare/wrangler.toml` | Configuração Wrangler versionada com produção e homologação separadas. |
...
Use o `cloudflare/wrangler.toml` versionado como fonte de verdade. Ele ja aponta producao para `db` e homologacao para `db-homologacao`; segredos continuam fora do Git e devem ser aplicados com `wrangler secret put`.
...
O Compose configura 5 sessões locais concorrentes, fila para 30 conexões aguardando e timeout de 20 minutos. A capacidade principal extra fica no Modal turbo; acompanhe CPU, RAM e fila no painel master antes de ampliar a concorrência local. A referência oficial das variáveis está na [documentação Docker do Browserless](https://docs.browserless.io/enterprise/docker/config).

Para somar capacidade externa sem sobrecarregar o Browserless local, configure `BROWSER_CDP_POOL` no formato `label|capacidade|url`, separado por `;;`. Exemplo: `browserless-local|5|ws://browserless:3000?token=...;;modal-turbo|32|wss://...modal.run?token=...`. Quando o pool estiver ativo, a API distribui novas sessões por peso. Em produção, o limite atual e `5 locais + 32 Modal com proxy = 37 navegadores`, protegido por `MAX_BROWSER_LIMIT`.
