...
HF_QWEN_PROVIDER = SOURCE_ROOT / "hf_qwen_provider.py"
CHROME_WRAPPER = Path(__file__).with_name("chrome_modal_no_sandbox.sh")

if modal.is_local():
    for required in (LEGACY_SOLVER, GOOGLE_SOLVER, GOOGLE_CLIENT, HF_GOOGLE_PROVIDER, HF_QWEN_PROVIDER, DETECTOR, ARTIFACT_RETENTION, CHROME_WRAPPER):
...
image = (
    ...
    .add_local_file(HF_QWEN_PROVIDER, "/app/hf_qwen_provider.py")
...
def solver_server() -> None:
    ...
    try:
        configured_max_solve_seconds = int(
            os.environ.get("PORTAL_MODAL_MAX_SOLVE_SECONDS", "360")
        )
    except (TypeError, ValueError):
        configured_max_solve_seconds = 360
    # Estados sem conteudo renovam o navegador cedo. Os 360 s evitam pagar uma
    ...
    max_solve_seconds = str(max(30, min(360, configured_max_solve_seconds)))
    solver_command = [
            "xvfb-run",
            "-a",
            "-s",
